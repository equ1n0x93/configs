...
docker exec -i $CONTAINER bash -c "cd malia && ./run_tests.sh -n 4 --durations=100 --cov-report term-missing --cov=./ --cov-config .coveragerc --cov-report=xml --junitxml=./results.xml"
cleanup
